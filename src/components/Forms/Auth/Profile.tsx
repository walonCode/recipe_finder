...
const Profile = () => {
  ...
  const filterFood = allFoods.filter(food => food.username === user?.username)

  const allRating = useAppSelector(selectAllRating)
  const filterRating = allRating.filter(rating => rating.username === user?.username)

  const allVotes = useAppSelector(getAllVote)
  const filterVote = allVotes.filter(vote => vote.username === user?.username)
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  ...
                  <Badge variant="outline" className="flex items-center gap-1">
                    ...
                    <span>Joined {new Date(user?.createdAt as string).toLocaleDateString()}</span>
                  </Badge>
                </div>
              </div>
